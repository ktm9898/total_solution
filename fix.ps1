$idxContent = Get-Content -Path "index.html" -Raw -Encoding UTF8
$admContent = Get-Content -Path "admin.html" -Raw -Encoding UTF8

# Extract CSS from index.html (string matching)
$idxStart = $idxContent.IndexOf("        .print-only {")
$idxEnd = $idxContent.IndexOf("        .ai-text-header {", $idxStart)
$idxCss = $idxContent.Substring($idxStart, $idxEnd - $idxStart)

# Extract CSS from admin.html
$admStart = $admContent.IndexOf("        /* ======================== */`n        /* 인쇄 전용 CSS")
if ($admStart -lt 0) {
    $admStart = $admContent.IndexOf("        /* ======================== */`r`n        /* 인쇄 전용 CSS")
}
$admEnd = $admContent.IndexOf("        .ai-text-header {", $admStart)

$admContent = $admContent.Substring(0, $admStart) + $idxCss + $admContent.Substring($admEnd)

# Replace printSurvey function
$funcStart = $admContent.IndexOf("            // 설문지 인쇄 (index.html 폼 구조 100% 동일)")
if ($funcStart -lt 0) {
    $funcStart = $admContent.IndexOf("            const printSurvey = (record) => {")
}
# Find next element
$funcEnd = $admContent.IndexOf("            // 데이터 로드", $funcStart)
...
$newSurvey = @"
            const printSurvey = (record) => {
                    ...
                    for (let qIdx = 0; qIdx < cat.count; qIdx++) {
                        ...
                        const scoreCells = [1, 2, 3, 4].map(s => {
                            ...
                            return `<td class="p-4 text-center">
                                <div class="w-9 h-9 rounded-sm border transition-all text-sm font-black ` + (isSelected ? 'bg-[#0055A5] border-[#0055A5] text-white shadow-md' : 'bg-white border-slate-200 text-slate-300') + `" style="display:inline-flex;align-items:center;justify-content:center;-webkit-print-color-adjust:exact;print-color-adjust:exact;">` + s + `</div>
                            </td>`;
                        }).join('');
                ...
                const benefitGrid = benefitLabels.map(item => {
                    ...
                    return `<label class="flex items-start gap-3 px-4 py-2.5 rounded-sm border transition-all cursor-pointer group ` + (checked ? 'bg-blue-50 border-blue-200' : 'bg-white border-slate-100 hover:bg-slate-50') + `" style="-webkit-print-color-adjust:exact;print-color-adjust:exact;">
                ...
                printArea.className = '';
                printArea.innerHTML = `
                    <div class="min-h-screen py-8 px-4 relative bg-slate-100 print:bg-white print:p-0 print:m-0">
                        <div class="max-w-5xl mx-auto bg-white shadow-2xl border border-slate-200 overflow-hidden print-container rounded-sm">
                            ...
                            <div class="p-10 space-y-12">
                                        ...
                                        <div class="flex items-center gap-5">
                                            ` + ["지속적인 감소", "감소 후 정체", "현상유지", "증가 후 정체", "지속적인 증가"].map(opt => {
                                                ...
                                                return `<label class="flex items-center gap-2 cursor-pointer group">
                                                    <input type="radio" name="revenue" ` + (isChecked ? 'checked' : '') + ` disabled class="w-4 h-4 accent-[#0055A5]" style="-webkit-print-color-adjust:exact;print-color-adjust:exact;" />
                                ...
                                <section>
                                    ...
                                    <div class="legend-box mb-5 flex flex-wrap gap-5 bg-blue-50/50 p-4 border border-blue-100/50 rounded-sm shadow-sm" style="-webkit-print-color-adjust:exact;print-color-adjust:exact;">
                                ...
                                <div class="bottom-grid grid grid-cols-1 md:grid-cols-2 gap-10 items-stretch">
                                    ...
                                    <section class="flex flex-col">
                                        ...
                                        <div class="notes-area w-full flex-grow p-5 border border-slate-200 rounded-sm text-base bg-slate-50/50 outline-none resize-none font-medium leading-relaxed whitespace-pre-wrap">` + notes + `</div>
                                    </section>
                                </div>
                            </div>
                        </div>
...
$admContent = $admContent.Substring(0, $funcStart) + $newSurvey + $admContent.Substring($funcEnd + 13)

Set-Content -Path "admin.html" -Value $admContent -Encoding UTF8
Write-Output "Done"
