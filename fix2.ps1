...
# Find next element
$funcEnd = $admContent.IndexOf("            // ", $funcStart)
if ($funcEnd -eq -1) { $funcEnd = $admContent.IndexOf("            const AdminApp", $funcStart) }
                    ...
                    for (let qIdx = 0; qIdx < cat.count; qIdx++) {
                        ...
                        const scoreCells = [1, 2, 3, 4].map(s => {
                            ...
                            return `<td class="p-4 text-center">
                                <button class="w-9 h-9 rounded-sm border transition-all text-sm font-black ` + (isSelected ? 'bg-[#0055A5] border-[#0055A5] text-white shadow-md' : 'bg-white border-slate-200 text-slate-300') + `">` + s + `</button>
                            </td>`;
                        }).join('');
...
$admContent = $admContent.Substring(0, $funcStart) + $newSurvey + $admContent.Substring($funcEnd + 15)
